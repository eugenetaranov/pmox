# interactive-target-picker Specification

## Purpose

A shared interactive VM-picker helper used by every pmox command that takes a `<name|vmid>` positional argument, plus the TTY-detection and auto-select-single rules that govern when it activates. Makes single-target commands (`shell`, `exec`, `delete`, `start`, `stop`, `info`) work without forcing the user to retype a VM name they can already see from `pmox list`.
