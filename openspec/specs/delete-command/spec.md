...
When the picker auto-selects the only pmox VM on the cluster, the confirmation prompt SHALL still run (unless `--yes` / `PMOX_ASSUME_YES` bypasses it). The one-line summary is the only safety net at that point; skipping it because "there was only one VM" would be a data-loss footgun.

#### Scenario: Delete without a target, single pmox VM
- **WHEN** `pmox delete` is invoked with no positional argument
- **AND** exactly one pmox-tagged VM exists
- **THEN** the command SHALL auto-select that VM without showing a picker
- **AND** SHALL still run the confirmation prompt before any destructive call

#### Scenario: Delete without a target, multiple pmox VMs, interactive TTY
- **WHEN** `pmox delete` is invoked with no positional argument
- **AND** two or more pmox-tagged VMs exist
- **AND** stdin and stderr are both TTYs
- **THEN** the command SHALL display the target picker
- **AND** SHALL then run the confirmation prompt against the selected VM

#### Scenario: Delete without a target, non-TTY
- **WHEN** `pmox delete` is invoked with no positional argument and stdin is a pipe
- **THEN** the command SHALL exit non-zero without drawing any UI
- **AND** SHALL NOT issue any destructive API call

#### Scenario: Delete without a target, zero pmox VMs
- **WHEN** `pmox delete` is invoked with no positional argument
