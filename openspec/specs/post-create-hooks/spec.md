...
Post-SSH-ready hook surface for `pmox launch` and `pmox clone`. Once the launcher has confirmed sshd is reachable, it hands off to a user-chosen tool — a shell script (`--post-create`), a tack config (`--tack`), or an Ansible playbook (`--ansible`) — to do post-boot orchestration. Failure semantics are configurable via `--strict-hooks`.
