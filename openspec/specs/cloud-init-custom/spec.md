...
Cloud-init custom-snippet surface for `pmox launch` and `pmox clone`.
pmox owns a per-server cloud-init file on the operator's machine,
uploads it as a Proxmox `snippets` volume at launch time, and points
the new VM's `cicustom` at it. Snippet storage is resolved
independently from the VM disk storage so a cluster with block-only
disk pools plus a dir-backed snippet pool works out of the box.
## Requirements
